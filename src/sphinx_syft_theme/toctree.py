...
def add_inline_math(node: Node) -> str:
    ...
    math outputs are ignored by sphinx-syft-theme's header.
...
def add_toctree_functions(
    ...
    def suppress_sidebar_toctree(startdepth: int = 1, **kwargs):
        ...
        if ancestorname is None:
            # Check if this is really the root toctree
            if pagename == app.config.root_doc:
                # Use the theme option to determine whether to show the sidebar
                return not context.get("theme_show_sidebar_on_root_doc", False)
                # return False # Don't suppress for the root toctree
            return True  # suppress for other pages with no ancestor
