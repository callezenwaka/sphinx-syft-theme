"""Bootstrap-based sphinx theme from the Syft community."""
...
def update_config(app):
    ...
    # Validate text links
    if not isinstance(theme_options.get("text_links", []), list):
        raise ExtensionError(
            "`text_links` must be a list of dictionaries, you provided "
            f"type {type(theme_options.get('text_links'))}."
        )

    # Set the anchor link default to be # if the user hasn't provided their own
    if not utils.config_provided_by_user(app, "html_permalinks_text"):
        app.config.html_permalinks_text = "#"
...
def update_and_remove_templates(
    ...
    for section in template_sections:
        if context.get(section):
            context[section] = utils._update_and_remove_templates(
                ...
                section=section,
                templates_skip_empty_check=["sidebar-nav-bs.html", "navbar-menu.html"],
            )
    ...
    if "css_files" in context:
        theme_css_name = "_static/styles/sphinx-syft-theme.css"
...
def setup(app: Sphinx) -> Dict[str, str]:
    ...
    # Bump priority so that it runs after the Syft theme sets up the edit URL func.
    app.connect("html-page-context", add_launch_buttons)
    app.connect("html-page-context", add_source_buttons)
    app.connect("html-page-context", add_header_buttons)
