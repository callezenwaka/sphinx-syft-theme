...
from .components.persona import Persona, set_shortcode_to_image
...
def copy_config_images(app):
    if not hasattr(app.config, "html_theme_options"):
        return
    ...
    if "footer_logos" in config:
        ...
        for key, value in logos_config.items():
            ...
            if image:
                logos_config[key] = copy_persona_image(app, image)
...
def copy_persona_image(app, image):
    conf_dir = Path(app.confdir)
    out_dir = Path(app.outdir) / "_images"
    os.makedirs(out_dir, exist_ok=True)
    source_path = conf_dir / image
    
    if source_path.exists():
        destination_path = out_dir / source_path.name
        shutil.copy(source_path, destination_path)
        return str(Path("_images") / source_path.name)
    else:
        logger.warning(f"Image file not found: {source_path}")

def copy_custom_images(app):
    if not hasattr(app.config, "html_theme_options"):
        return

    config = app.config.html_theme_options
    custom_image_paths = config.get("custom_image_paths", [])
    ...
    for image_path in custom_image_paths:
        source_path = conf_dir / image_path
        if source_path.exists():
            destination_path = out_dir / source_path.name
            shutil.copy(source_path, destination_path)
        else:
...
def setup(app: Sphinx):
    ...
    app.add_html_theme("sphinx_syft_theme", get_html_theme_path())
    app.connect("builder-inited", copy_config_images)
    app.connect("builder-inited", copy_custom_images)

    custom_shortcode_to_image = app.config.html_theme_options.get('custom_shortcode_to_image', {})
    set_shortcode_to_image(custom_shortcode_to_image)
